		...
		case State::UNLOADED:
		{
			...
			if (!ResourcesCache::instance().loadRom())
			{
				...
				if (!gameProfile.mRomInfos.empty())
				{
					...
					const std::string text = "This game requires an original " + gameProfile.mRomInfos[0].mSteamGameName + " ROM, or the PC Collection version (as its executable, or a .iso/.zip containing it) to work.\nIf you have one, click OK and select it in the following dialog.\n\nSee the Manual for details.";
					...
					const std::wstring title = L"Select " + String(gameProfile.mRomInfos[0].mSteamGameName).toStdWString() + L" ROM, or PC Collection Executable / ISO / ZIP";
					const std::wstring romPath = PlatformFunctions::openFileSelectionDialog(title, gameProfile.mRomInfos[0].mSteamRomName, L"Game Files (*.bin;*.exe;*.iso;*.zip)\0*.bin;*.exe;*.iso;*.zip\0Genesis ROM files (*.bin)\0*.bin\0PC Collection Executable (*.exe)\0*.exe\0PC Collection Disc Image (*.iso)\0*.iso\0PC Collection Archive (*.zip)\0*.zip\0All Files\0*.*\0\0");
